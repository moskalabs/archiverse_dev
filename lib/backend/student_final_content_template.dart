...
/// 학생별 기말결과물 콘텐츠 템플릿
class StudentFinalContentTemplate {
  ...
  /// 한 학생의 기말결과물 자료 추가
  static Future<void> addStudentFinalContent({
    ...
    try {
      print('======= $studentName 기말결과물 자료 추가 시작 =======');
      
      final font = await _loadMalgunGothicFont();
      
      // final_results 테이블에서 해당 학생의 기말결과물 조회
      ...
      print('$studentName 기말결과물 개수: ${finalResults.length}개');
      
      if (finalResults.isEmpty) {
        print('$studentName 기말결과물 없음 - 스킵');
        return;
      }
      ...
      for (int i = 0; i < finalResults.length; i++) {
        ...
        final title = result.title ?? '최종결과물';
        
        if (pdfUrl == null || pdfUrl.isEmpty) {
          print('$studentName 기말결과물 URL 없음 - 스킵');
          continue;
        }
        
        // 진행률 업데이트
        final progressValue = startProgress + (i / finalResults.length) * (endProgress - startProgress);
        updateProgress(progressValue, '$studentName 기말결과물 처리 중 (${i + 1}/${finalResults.length})');
        
        print('$studentName 기말결과물 PDF 다운로드 중: $pdfUrl');
        
        try {
          final response = await http.get(Uri.parse(pdfUrl));
          if (response.statusCode != 200) {
            print('$studentName 기말결과물 다운로드 실패: ${response.statusCode}');
            continue;
          }
          ...
          print('$studentName 기말결과물 페이지 수: ${resultDoc.pages.count}');
          
          // 기말결과물의 각 페이지마다 템플릿 생성
          for (int pageIndex = 0; pageIndex < resultDoc.pages.count; pageIndex++) {
                      ...
                      children: [
                        pw.Row(children: [
                          pw.Container(
                            ...
                            child: pw.Center(child: pw.Text('08', style: pw.TextStyle(
                              fontSize: 16, color: PdfColors.white, fontWeight: pw.FontWeight.bold, font: font,
                            ))),
                          ),
                          pw.SizedBox(width: 15),
                          pw.Text('최종 결과물 (${professorName ?? '교수명'} 교수)', style: pw.TextStyle(
            ...
            print('$studentName 기말결과물 페이지 ${pageIndex + 1}/${resultDoc.pages.count} 템플릿 기반 병합 완료');
          }
          
          resultDoc.dispose();
          print('$studentName 기말결과물 PDF 병합 완료');
          
        } catch (resultError) {
          print('$studentName 기말결과물 처리 오류: $resultError');
        }
      }
      
      updateProgress(endProgress, '$studentName 기말결과물 완료');
      print('======= $studentName 기말결과물 자료 추가 완료 =======');
      
    } catch (e) {
      print('$studentName 기말결과물 자료 추가 실패: $e');
    }
  }
}